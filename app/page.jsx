...
function bouwModel(train,{halvering,weeg}){const nu=Date.now(),MIN_W=15;const matchW=(r)=>{let w=cgew(r.toernooi,weeg);if(r.datum){const lj=(nu-r.datum.getTime())/(365.25*864e5);if(lj>=0)w*=.5**(lj/halvering);}return w>0?w:0};const finRating=(T,league)=>{const rating={};for(const[t,d]of Object.entries(T)){const conf=Math.min(1,d.w/MIN_W);const rawA=d.wA>0?d.vo/d.wA:league,rawD=d.wD>0?d.te/d.wD:league;rating[t]={aanval:conf*rawA+(1-conf)*league,verdediging:conf*rawD+(1-conf)*league};}return rating;};const accum=(rating,league,adj,ha)=>{const T={};let tg=0,tw=0;for(const r of train){if(!isScore(r.thuisScore)||!isScore(r.uitScore))continue;const w=matchW(r);if(!w)continue;const hs=+r.thuisScore,as=+r.uitScore;const rh=rating[r.thuis]||{aanval:league,verdediging:league},ru=rating[r.uit]||{aanval:league,verdediging:league};const cI=(v)=>Math.max(.55,Math.min(1.65,league/Math.max(.35,v))),cD=(v)=>Math.max(.55,Math.min(1.65,v/league));const fDU=adj?cI(ru.verdediging):1,fAU=adj?cD(ru.aanval):1,fDH=adj?cI(rh.verdediging):1,fAH=adj?cD(rh.aanval):1;for(const t of[r.thuis,r.uit])if(!T[t])T[t]={w:0,wA:0,vo:0,wD:0,te:0};T[r.thuis].w+=w;T[r.uit].w+=w;T[r.thuis].wA+=w*fDU;T[r.thuis].vo+=w*hs*fDU;T[r.thuis].wD+=w*fAU;T[r.thuis].te+=w*as*fAU;T[r.uit].wA+=w*fDH;T[r.uit].vo+=w*as*fDH;T[r.uit].wD+=w*fAH;T[r.uit].te+=w*hs*fAH;tg+=w*(hs+as);tw+=w*2;if(ha&&!r.neutraal){ha.nt+=w*hs;ha.nu+=w*as;ha.nw+=w;}}return{T,tg,tw};};const ha={nt:0,nu:0,nw:0};let{T,tg,tw}=accum({},1.3,false,ha);let league=tw>0?tg/tw:1.3;let rating=finRating(T,league);({T,tg,tw}=accum(rating,league,true,null));rating=finRating(T,league);({T,tg,tw}=accum(rating,league,true,null));league=tw>0?tg/tw:league;rating=finRating(T,league);const normIdx={};Object.keys(rating).forEach((k)=>(normIdx[norm(k)]=k));let thuisv=1.1;if(ha.nw>0&&ha.nu>0)thuisv=Math.min(1.4,Math.max(1,Math.sqrt((ha.nt/ha.nw)/(ha.nu/ha.nw))));return{rating,normIdx,league,thuisv};}
