...
function bouwModel(train,{halvering,weeg}){const nu=Date.now();const matchW=(r)=>{let w=cgew(r.toernooi,weeg);if(r.datum){const lj=(nu-r.datum.getTime())/(365.25*864e5);if(lj>=0)w*=.5**(lj/halvering);}return w>0?w:0};const mean=(xs)=>xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:1.3;const clampF=(x)=>Math.max(.55,Math.min(1.65,x));const T1={};let tg=0,tw=0,ha={nt:0,nu:0,nw:0};for(const r of train){if(!isScore(r.thuisScore)||!isScore(r.uitScore))continue;const w=matchW(r);if(!w)continue;const hs=+r.thuisScore,as=+r.uitScore;for(const t of[r.thuis,r.uit])if(!T1[t])T1[t]={w:0,vo:0,te:0};T1[r.thuis].w+=w;T1[r.thuis].vo+=w*hs;T1[r.thuis].te+=w*as;T1[r.uit].w+=w;T1[r.uit].vo+=w*as;T1[r.uit].te+=w*hs;tg+=w*(hs+as);tw+=w*2;if(!r.neutraal){ha.nt+=w*hs;ha.nu+=w*as;ha.nw+=w;}}const league=tw>0?tg/tw:1.3;const R1={};for(const[t,d]of Object.entries(T1))R1[t]={aanval:d.w>0?d.vo/d.w:league,verdediging:d.w>0?d.te/d.w:league};const muA1=mean(Object.values(R1).map((x)=>x.aanval)),muD1=mean(Object.values(R1).map((x)=>x.verdediging));const T2={};for(const r of train){if(!isScore(r.thuisScore)||!isScore(r.uitScore))continue;const w=matchW(r);if(!w)continue;const hs=+r.thuisScore,as=+r.uitScore;const rh=R1[r.thuis]||{aanval:league,verdediging:league},ru=R1[r.uit]||{aanval:league,verdediging:league};const fScU=clampF(muD1/Math.max(.35,ru.verdediging)),fScH=clampF(muD1/Math.max(.35,rh.verdediging)),fTeU=clampF(muA1/Math.max(.35,ru.aanval)),fTeH=clampF(muA1/Math.max(.35,rh.aanval));for(const t of[r.thuis,r.uit])if(!T2[t])T2[t]={wA:0,vo:0,wD:0,te:0};T2[r.thuis].wA+=w*fScU;T2[r.thuis].vo+=w*hs*fScU;T2[r.thuis].wD+=w*fTeU;T2[r.thuis].te+=w*as*fTeU;T2[r.uit].wA+=w*fScH;T2[r.uit].vo+=w*as*fScH;T2[r.uit].wD+=w*fTeH;T2[r.uit].te+=w*hs*fTeH;}const R2={};for(const[t,d]of Object.entries(T2))R2[t]={aanval:d.wA>0?d.vo/d.wA:league,verdediging:d.wD>0?d.te/d.wD:league};const muA2=mean(Object.values(R2).map((x)=>x.aanval)),muD2=mean(Object.values(R2).map((x)=>x.verdediging));const sA=muA2>0?muA1/muA2:1,sD=muD2>0?muD1/muD2:1;const rating={};for(const[t,r]of Object.entries(R2))rating[t]={aanval:r.aanval*sA,verdediging:r.verdediging*sD};for(const[t,r]of Object.entries(R1))if(!rating[t])rating[t]={aanval:r.aanval,verdediging:r.verdediging};const normIdx={};Object.keys(rating).forEach((k)=>(normIdx[norm(k)]=k));let thuisv=1.1;if(ha.nw>0&&ha.nu>0)thuisv=Math.min(1.4,Math.max(1,Math.sqrt((ha.nt/ha.nw)/(ha.nu/ha.nw))));return{rating,normIdx,league,thuisv};}
